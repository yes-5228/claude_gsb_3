...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 化粪池清掏外运** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。

## 功能模块

| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布、区域运行情况、重点关注公厕、化粪池清掏到期预警、最新问题与巡查 |
| 公厕台账 | `/restrooms`、`/restrooms/:id` | 台账增删改查（含化粪池池容与使用频次）、区域与状态筛选、公厕详情（档案 + 历史巡查 + 历史问题 + 清掏记录）、关联数据删除保护 |
...
| 清掏台账 | `/septic` | 登记清掏日期/作业单位/清掏量/外运去向（含车牌与联单）、按池容与使用频次推算清掏周期、到期前 7 天提醒、超期未清掏公厕单独列出、本月作业量统计 |
...
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、清掏记录
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、评分、清掏周期、统计
│   │   ├── seed.py              # 演示数据生成
...
│   │   ├── pages                # dashboard / restrooms / inspections / issues / septic 五个模块
│   │   ├── utils                # 时间格式化、评分换算
│   │   └── styles/global.css
│   ├── nginx.conf
│   └── Dockerfile
...
| GET | `/septic/overview` | 清掏台账指标（本月作业量、超期/即将到期数量） |
| GET | `/septic/schedules` | 各公厕清掏计划与预警（district/status/overdue_only/keyword） |
| GET | `/septic/cleanings` | 清掏外运台账查询（公厕/区域/作业单位/去向/日期区间/关键字） |
| POST | `/septic/cleanings` | 登记清掏记录，自动生成编号 `HC-YYYYMMDD-001` |
| GET/PATCH/DELETE | `/septic/cleanings/{id}` | 详情 / 更新 / 删除 |
...
- **清掏周期推算**：清掏周期（天）= 化粪池容积(m³) × 18 ÷ 使用频次系数（低频 0.6 / 中频 1.0 / 高频 1.6），结果夹取在 15-180 天之间；公厕档案中手工填写周期时以人工值为准。下次清掏日期 = 最近一次清掏日期 + 周期；距到期 ≤ 7 天标记「即将到期」，已过下次清掏日期标记「已超期」，从无清掏记录标记「未建档」。
- **清掏到期提醒**：清掏台账页与总览看板对「即将到期」与「已超期」公厕单独列出（支持「超期未清掏」页签与 `overdue_only` 过滤），超期时页面顶部红色提示并可一键跳转。
- **删除保护**：删除公厕时若已存在巡查、问题或清掏记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态，并标注化粪池容积与使用频次）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹，以及约 20 条化粪池清掏外运记录（覆盖正常/即将到期/已超期/未建档四种状态）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、流程流转、清掏周期预警、统计）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：10 个用例全部通过；`/health`、台账/巡查/问题/清掏/统计/字典接口均返回预期数据。
